...
if (!in_array($fileExtension, $allowedExtensions)) {
    echo "<script>alert('Hanya file gambar yang diizinkan!'); window.location.href='../index.php?p=acc_baptis_bayi'</script>";
...
if ($gambarSize > $maxFileSize) {
    echo "<script>alert('Ukuran file terlalu besar! Maksimum 5MB.'); window.location.href='../index.php?p=acc_baptis_bayi'</script>";
...
// Perbarui data gambar pada tabel baptis_bayi
...
mysqli_query($conn, "UPDATE baptis_bayi SET dokumentasi = '$gambarName' WHERE id_user = '$id_user'");

echo "<script>alert('Data gambar berhasil diperbarui'); window.location.href='../index.php?p=acc_baptis_bayi'</script>";
